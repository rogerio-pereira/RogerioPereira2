<?php

return [

    /*
    |--------------------------------------------------------------------------
    | SEO Defaults
    |--------------------------------------------------------------------------
    |
    | Every public controller passes a `seo` array (title, description and
    | optionally image) that is merged over these defaults. Titles are passed
    | complete. See docs/02 HLD.md, section 2.
    |
    */

    'seo' => [
        'title' => 'Rogerio Pereira · Web systems that hold up in production',
        'description' => 'Senior full-stack engineer with 15+ years in production. Websites, systems, AI integration, automation and consulting, delivered as projects with a clear scope, start and finish.',
        'image' => '/og-image.jpg',
        'twitter_site' => '@rpereira_dev',
        'theme_color' => '#151719',
    ],

];
